...
<c:choose>
    <c:when test="${grid == '4_8'}">
        <div class="col-md-4 col-xl-3">${displayAbsoluteArea}<template:area path="${colName}-side" areaAsSubNode="true" moduleType="${moduleType}" level="${level}"/></div>
        <div class="col-md-8 col-xl-9">${displayAbsoluteArea}<template:area path="${colName}-main" areaAsSubNode="true" moduleType="${moduleType}" level="${level}"/></div>
    </c:when>
